...
liste_phrase_choix_langue = ["enter le nom de la langue que vous voulez utiliser :",
                             ...
                             "peut ètre que l'orthographe est mauvaise vous pouvez consulter la liste des langues "
                             "disponibles dans le menu"]

liste_phrase_menu = ["Pour voir les langues disponibles : entrez 4",
                     str("Il y a actuelement " + str(
                         nbmot) + " réponse a trouver : pour modifier le nombre de réponse à trouver entrez 5"),
...
def traduction_phrase_du_jeu():
    ...
    if langue_jeu != 'fr':
        ...
        liste_phrase_special[2] = str(trad_menu(
            "La langue du jeu est actuellement le " + constants.LANGUAGES[langue_jeu] + " : pour la changer entrez 3"))
        liste_phrase_special[1] = str(trad_menu(
            "La langue dans laquelle le mot doit ètre traduit est " + constants.LANGUAGES[
                langue_destination] + " : pour la changer entrez 2"))
        liste_phrase_special[0] = str(trad_menu(
            "La langue du mot proposé à la traduction est le " + constants.LANGUAGES[
                langue_source] + " : pour la changer entrez 1"))
        ...
        if choix == "5":
            if mode_jeu == "mot":
                ...
                if nbmot < 1:
                    ...
                    nbmot = 5
            liste_phrase_menu[1] = trad_menu(str("Il y a actuelement " + str(
                nbmot) + " réponses a trouver : pour modifier le nombre de réponses à trouver entrer 5"))
        if choix == "6":
            partie(nbmot)
        if choix == "3":
            temp_langue = choix_langue()
        ...
        if choix == "8":
            ...
            if choixdif == "1":
                if mode_jeu != "mot":
                    ...
                    difficulte = "aléatoire"
                    liste_phrase_special[3] = str(
                        trad_menu("les mots proposés sont générés aléatoirement , pour les modifier entrez 8"))

            elif choixdif == "2":
                difficulte = "simple"
                liste_phrase_special[3] = str(
                    trad_menu("les questions propsés font partie de la liste simple : pour les modifier entrez 8", ))
                print(str(trad_menu("les questions proposées ont bien été modifié")))
            elif choixdif == "3":
                difficulte = "moyen"
                liste_phrase_special[3] = str(
                    trad_menu("les questions propsées font partie de la liste moyenne : pour les modifier entrer 8", ))
                print(str(trad_menu("les questions proposées ont bien été modifié")))
            elif choixdif == "4":
                liste_phrase_special[3] = str(trad_menu(
                    "les questions propsées font partie de la liste difficile : pour les modifier entrez 8", ))
                print(str(trad_menu("les questions proposées ont bien été modifié")))
        ...
        if choix == "9":
            if mode_jeu == "mot":
                ...
                if difficulte == "aléatoire":
                    difficulte = "simple"
                    liste_phrase_special[3] = str(trad_menu(
                        "les questions proposées font partie de la liste simple : pour les modifier entrez 8", ))
            else:
                mode_jeu = "mot"
        ...
        while 0 <= nbechec < 3:
            ...
            if mode_jeu == "mot":
                ...
                if nbechec == 3:
                    ...
                    motactuel = motactuel + 1
                    print(str(trad_menu(
                        "votre meilleurs socre pour cette phrase était de " + str(int(max_score_essaie)))))
                    tab_score.append(max_score_essaie)
                    max_score_essaie = 0
    if mode_jeu == "mot":
        print(liste_phrase_jeu[7] + " " + str(nbbonnereponse) + " " + liste_phrase_jeu[1] + " " + liste_phrase_jeu[
            8] + " " + str(nbmauvaisereponse) + " " + liste_phrase_jeu[3])
    else:
        score_total = 0
        ...
        print(str(trad_menu(
            "votre score moyen pour cette partie est de " + str(int(score_total / len(tab_score))) + "% de réussite")))
    menu()
